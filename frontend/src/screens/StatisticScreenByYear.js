...
function StatisticScreenByYear() {
  ...
  const handleGetYear = (str) => {
    const arr = str.split("T");
    let objectDate = new Date(arr[0]);
    return objectDate.getFullYear().toString();
  };
  const handleStatistic = function () {
    ...
    orders = orderMain.current.filter((item) => {
        console.log(handleGetYear(item.createAt))
      return date === handleGetYear(item.createAt);
    });
  }
  else {
...
export default StatisticScreenByYear;
